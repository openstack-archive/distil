...
class DistilPeriodicTasks(periodic_task.PeriodicTasks):
    @periodic_task.periodic_task(spacing=CONF.collector.collect_usage_interval,
                                 run_immediately=True)
    def collect_usage(self, ctx=None):
        """Run usage collection on all tenants present in Keystone."""
        try:
            LOG.debug("Usage collection run started.")
            ...
            LOG.debug("Usage collection run complete.")
            # LOG.debug("resp: %s", json.dumps(resp))
