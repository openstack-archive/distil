...
class BaseCollector(object):
    ...
    def collect_usage(self, project, start, end):
        ...
        for window_start, window_end in windows:
            LOG.info("%s %s slice %s %s" % (project.id, project.name,
            ...
            try:
                for mapping in CONF.collector.meter_mappings:
                ...
                LOG.warning(
                    "IntegrityError for %s(%s) in window: %s - %s ",
    ...
    def _filter_and_group(self, usage, usage_by_resource):
        ...
        for u in usage:
            ...
            if (trust_sources and all([not re.match(source, u['source'])
    ...
    def _transform_usages(self, project_id, usage_by_resource, mapping,
        ...
        for res_id, entries in usage_by_resource.items():
            ...
            if transformed:
                ...
                for service, volume in transformed.item():
                    entry = {
                        'service': service,
                        'volume': volume,
                        'unit': mapping['unit'],
